...
def test_dashboard_primary_actions_include_email_sync(page: Page, web_base_url: str) -> None:
    ...
    expect(page.locator("#job-ingestion-submenu")).to_be_visible()
    expect(page.locator("#email-sync-nav")).to_be_visible()
    expect(page.locator("#import-url-nav")).to_contain_text("Automated Job Import")
    expect(page.locator("#primary-actions-card").get_by_role("button", name="Job Discovery")).to_be_visible()
    expect(page.locator("#primary-actions-card").get_by_role("button", name="Sync Email Alerts")).to_be_visible()
    page.locator("#primary-actions-card").get_by_role("button", name="Sync Email Alerts").click()
